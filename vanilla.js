document.addEventListener('DOMContentLoaded', () => {
  ...
  updateVanillaRanking();
});

function updateVanillaRanking() {
  const users = getUsers();
  const rankingList = document.getElementById('vanillaRankingList');
  rankingList.innerHTML = '';

  const rankedUsers = users
    .filter(user => user.vanilla && user.vanilla.length > 0)
    .map(user => {
      return { username: user.username, tiers: user.vanilla, region: 'NA' };
    })
  ...
  rankedUsers.forEach((user, index) => {
    ...
    li.innerHTML = `
      ...
      <div class="player-info">
        ...
        <span class="ml-2">${user.username} <span class="text-green-400">Vanilla Expert (${user.tiers.length * 100} points)</span></span>
      </div>
...
function showPlayerDetail(username, tiers, region, index) {
  ...
  content.innerHTML = `
    <p>Position: ${index + 1}. Vanilla (${tiers.length * 100} points)</p>
    <p>Region: ${region}</p>
