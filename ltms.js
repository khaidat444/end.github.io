...
function updateLTMSRanking() {
  const users = getUsers();
  const rankingList = document.getElementById('ltmsRankingList');
  rankingList.innerHTML = '';

  const rankedUsers = users
    .filter(user => user.ltms && user.ltms.length > 0)
    .map(user => {
      return { username: user.username, tiers: user.ltms, region: 'NA' };
    })
  ...
  rankedUsers.forEach((user, index) => {
    ...
    li.innerHTML = `
      ...
      <div class="player-info">
        ...
        <span class="ml-2">${user.username} <span class="text-teal-400">LTMS Pro (${user.tiers.length * 100} points)</span></span>
      </div>
...
function showPlayerDetail(username, tiers, region, index) {
  ...
  content.innerHTML = `
    <p>Position: ${index + 1}. LTMS (${tiers.length * 100} points)</p>
    <p>Region: ${region}</p>
